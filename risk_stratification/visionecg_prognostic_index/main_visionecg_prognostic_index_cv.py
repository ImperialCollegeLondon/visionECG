#!/usr/bin/env python3
"""Run visionECG prognostic nested cross-validation."""
...
from config_visionecg_prognostic_index import (
    CONFIG_NAME,
...
from loader_visionecg_prognostic_index import load_event_cohort, resolve_prefixed_path
from model_visionecg_prognostic_index import (
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="XGBoost + Optuna nested 5-fold CV for the visionECG prognostic index."
    )
    ...
    p.add_argument("--measurement_cols", nargs="+", type=str, required=True,
                   help="Feature column names (bash supplies the full list).")
...
def main() -> None:
    ...
    feature_cols = list(args.measurement_cols)
    if not feature_cols:
        raise ValueError("--measurement_cols must be a non-empty list.")
