...
from loader_ecg_parameters_xgboost import ECGParametersXGBoostDataModule
from model_ecg_parameters_xgboost import (
    compute_metrics,
...
def setup_logging(checkpoint_dir: str):
    ...
    log_file = os.path.join(log_dir, 'ecg_parameters_xgboost_wrapper.log')

    logger = logging.getLogger('ecg_parameters_xgboost')
    ...
    logger.info("="*80)
    logger.info("ECG-parameters XGBoost multi-label one-vs-rest classification")
    logger.info("="*80)
...
def train_single_comparison(
    ...
    data_module = ECGParametersXGBoostDataModule(
        train_csv=train_csv,
        ...
        atrial_features=atrial_features,
...
def main():
    parser = argparse.ArgumentParser(
        description="ECG-parameters XGBoost multi-label one-vs-rest classification",
    ...
    parser.add_argument("--ecg_phenotypes_path", type=str, required=True,
                        help="CSV with per-instance ECG phenotype columns keyed by eid_40616.")
    parser.add_argument("--checkpoint_dir", type=str, required=True)
    parser.add_argument("--output_prefix", type=str, default="ecg_parameters_xgboost_")
    parser.add_argument(
    ...
    parser.add_argument("--morphology_cols", nargs='+', type=str, required=True,
                        help="ECG parameter columns (features) present in --ecg_phenotypes_path.")
    parser.add_argument("--atrial_features", nargs='*', type=str, default=[],
                        help="Subset of morphology columns filled with 0 (e.g. AF cases lacking P wave).")
    parser.add_argument(
    ...
    parser.add_argument("--strategies", nargs='+', type=str, default=['spw_only'],
                        choices=['smote_only', 'spw_only', 'smote_spw', 'none'],
    ...
    logger.info(f"ECG phenotypes: {args.ecg_phenotypes_path}")
    ...
    logger.info(f"Morphology cols ({len(args.morphology_cols)}): {args.morphology_cols}")
    logger.info(f"Atrial features ({len(args.atrial_features)}): {args.atrial_features}")
    ...
    for i, disease in enumerate(args.diseases, 1):
        ...
        try:
            result = train_single_comparison(
                ...
                morphology_cols=args.morphology_cols,
                atrial_features=args.atrial_features,
                virtual_label_map=virtual_label_map,
            )
    ...
    logger.info("\n" + "="*80)
    logger.info("ECG-parameters XGBoost CLASSIFICATION COMPLETED")
    logger.info("="*80)
    logger.info(f"Checkpoint dir: {checkpoint_dir}")
