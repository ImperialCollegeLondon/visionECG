#!/usr/bin/env python3
"""Bootstrap disease-split metrics using validation-derived Youden thresholds."""
...
def metrics_at_threshold(y: np.ndarray, p: np.ndarray, thr: float) -> dict:
    """Compute threshold metrics with Haldane continuity correction."""
...
def bca_ci_generic(y, p, metric_fn, n_boot=2000, random_seed=42, ci_level=0.95):
    y = np.asarray(y)
    p = np.asarray(p)
    try:
...
def bca_ci_fixed_threshold(y, p, thr, metric_name, n_boot=2000, random_seed=42, ci_level=0.95):
    def stat(y_r, p_r):
...
def bca_ci_val_refit(y, p, metric_name, n_boot=2000, random_seed=42, ci_level=0.95):
    def stat(y_r, p_r):
...
def bca_ci_log_dor(y, p, thr, n_boot=2000, random_seed=42, ci_level=0.95):
    def stat(y_r, p_r):
...
def evaluate_split(y, p, thr_val, split_name, n_boot, random_seed):
    rows = []
...
def process_model_dataset(
    ...
    if dataset_label not in diseases:
        raise ValueError(
            f"Unknown disease {dataset_label!r}; expected one of {diseases}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description='Multi-split BCa bootstrap analysis for binary disease prediction (label 6).',
    )
    parser.add_argument('--input_tables', nargs='+', type=str, required=True,
                        help='Per-model paths tables (columns: dataset_label, train_path, val_path, test_path).')
    ...
    parser.add_argument('--diseases', nargs='+', type=str, required=True,
                        help='Canonical disease labels (order-insensitive; validates dataset_label column).')
    parser.add_argument('--n_bootstraps', type=int, default=500)
    parser.add_argument('--random_seed', type=int, default=42)
    parser.add_argument('--n_jobs', type=int, default=96,
                        help='Parallel workers for BCa. 1 disables parallelism; -1 uses all cores.')
    ...
    print(f'Output: {args.output_dir} | n_boot={args.n_bootstraps} | seed={args.random_seed} | n_jobs={args.n_jobs}')
    ...
    for table_path, model_name, prob_col in zip(args.input_tables, args.model_names, args.prob_cols):
        print(f"\n=== {model_name} | {table_path} | prob_col={prob_col} ===")
        try:
