...
class TaskConfig:
    ...
    threshold: float
    sex_filter: int  # 0 = female, 1 = male
...
def stratified_split_and_write(
    ...
    """Create and save a sex-filtered stratified 80/20 split."""
...
def setup_logging(output_dir: str) -> logging.Logger:
    ...
    logger = logging.getLogger("echonext_mini_retrain_stratified")
...
def main():
    parser = argparse.ArgumentParser(description="EchoNext-Mini retrain — sex-stratified tasks")

    parser.add_argument("--task_key", type=str, required=True,
                        help="Task identifier, safe for filenames (e.g. LVEDVi_Male_greater_than_75)")
    parser.add_argument("--display_name", type=str, required=True,
                        help="Human-readable task label for logs (e.g. 'LVEDVi (male) > 75')")
    ...
    parser.add_argument("--sex_filter", type=int, required=True,
                        choices=[0, 1],
                        help="Restrict to one sex: 0 = female, 1 = male")
    ...
    parser.add_argument("--train_csv", type=str, required=True,
                        help="Training CSV (will be split 80/20 into train/val after sex filter)")
    ...
    sex_name = 'male' if task_config.sex_filter == 1 else 'female'
    ...
    logger.info(f"Sex filter: {sex_name} ({task_config.sex_filter})")
    ...
    train_split_path, val_split_path = stratified_split_and_write(
        ...
        sex_filter=task_config.sex_filter,
        out_dir=splits_dir,
        logger=logger,
    )
